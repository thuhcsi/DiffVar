"""
Adapted from https://github.com/NATSpeech/NATSpeech/blob/238165e8cd430531b69c484cabb032c1313ee73b/modules/tts/diffspeech/shallow_diffusion_tts.py
"""
...
class GaussianDiffusion(nn.Module):
    ...
    # def __init__(self, dict_size, hparams, out_dims=None):
        ...
        self.fs2 = FastSpeech2(preprocess_config, model_config)
        pre_trained_fs2_ckpt = torch.load(hparams["fs2_path"])
        self.fs2.load_state_dict(pre_trained_fs2_ckpt["model"], strict=True)
        for k, v in self.fs2.named_parameters():
    ...
    def forward(self, *inputs, **kwinputs):
        ...
        fs2_ret = self.fs2(*inputs, **kwinputs, skip_decoder = True)
        cond = fs2_ret[0].transpose(1, 2)
        if self.training:
            t = torch.randint(0, self.K_step, (b,), device=device).long()
            x = inputs[4]
            x = self.norm_spec(x)
            x = x.transpose(1, 2)[:, None, :, :]  # [B, 1, M, T]
            diff_loss = self.p_losses(x, t, cond)
            # nonpadding = (mel2ph != 0).float()
            # ret['diff_loss'] = self.p_losses(x, t, cond, nonpadding=nonpadding)
            return [diff_loss]
        else:
            t = self.K_step
            fs2_mels, postnet_output, mel_masks = self.fs2.decode(fs2_ret[0], fs2_ret[-3])
            _cond, _post, *fs2_ret, _mel_masks, src_lens, mel_lens = fs2_ret
            fs2_ret = (fs2_mels, postnet_output, *fs2_ret, mel_masks, src_lens, mel_lens)
